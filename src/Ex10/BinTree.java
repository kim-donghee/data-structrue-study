...
public class BinTree<K, V> {
	...
	public boolean remove(K key) {
		...
		boolean isLeftChild = true;		// p는 부모의 왼쪽 자신 노드인가.
		
		while(true) {
			if(p == null)						// 더 이상 진행하지 않으면
				return false;					// 그 키 값은 없습니다.
			int cond = comp(key, p.getKey());	// key완 노드 p의 키 값을 비교
			if(cond == 0) 						// 값으면
				break;							// 검색 성공
			else {
		...
		if(p.left == null) {					// 왼쪽 노드에 자식이 없음
			if(p == root) {
				root = p.right;
			}
			else if(isLeftChild)
				parent.left = p.right;			// 부모의 왼쪽쪽 포인터가 오른쪽 자식을 가리킴
			else
				parent.right = p.right;			// 부모의 오른쪽 포인터가 오른쪽 자식을 가리킴
		}
		else if(p.right == null) {				// 오른쪽 노드에 자식이 없음
			if(p == root) {
				root = p.left;
			}
			else if(isLeftChild)
				parent.left = p.left;			// 부모의 왼쪽쪽 포인터가 오른쪽 자식을 가리킴
			else
	...
	// node를 루트로 하는 서브 트리의 노드를 키 값과 오름차순으로 출력
